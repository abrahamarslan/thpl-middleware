...
`BigIntPKWithUUIDMixin` (adds the public `uuid`, uuid4 generated by Python),
`BigIntPKWithUUIDv7Mixin` (the same column, but a **database-generated** PG18 `uuidv7()` — time-ordered,
so its unique index is appended to; opt-in, used by the `tax` schema), `TimestampMixin` (`created_at`,
...
| Class | Gets | Tables |
|---|---|---|
| **ENTITY** | everything in §2 | users, user_profiles, roles, documents, document_files, document_links, files, media, tags, favorites, emails, zoho_currencies, tax.tax_components, zoho_locations, zoho_users, organizations (its `parent_id` replaces `organization_id`), geo.places, geo.place_links, geo.geofences, geo.place_relationships |
...
A fourth shape is legitimate and enforced by the same test: **scoped config rows** — `TenantScopedMixin`
(or `MultiTenantMixin`) + `AuditMixin` + `AppMetaMixin` + `TimestampMixin` + a soft-delete mixin, with no
`status`, `is_verified` or `row_version`. They pass because they carry the ledger columns and no
`row_version`. Use it for children whose lifecycle belongs to a parent entity
(`tax.tax_group_members`, `tax.organization_tax_components`, `tax.org_default_tax_preferences`,
`tax.tax_exemptions`); the GLOBAL `tax.gst_treatment_types` is allow-listed with its reason.
