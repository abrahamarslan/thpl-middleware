...
| `users/schema.py` | `UserProfileBase` (every editable field), `UserCreate`/`UserUpdate`/`UserOut` (secrets excluded), `UserListFilters` (`city`/`state`/`country` resolve through the address book, not the user row), `LocationUpdate` + `LiveLocationOut` (lat/lng on the wire, PostGIS geography in storage — converted at this boundary only), auth schemas (login, **login-OTP**, register, refresh, password flows; password fields use the policy `PasswordStr` type; reset/OTP take a single `identifier`), `CountryOut`, `CountryTimezoneOut`, and the self-service profile schemas (`UserSelfUpdate` allowlist, `UserMeOut`, `UserAddressOut`). |
